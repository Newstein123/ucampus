...
const IdeaDetails: React.FC = () => {
    ...
    const { data: ideaResponse, isLoading } = useContributionDetailQuery(slug || '');
    const idea = ideaResponse?.data;
    // Derive id from idea for backwards compatibility
    const id = idea?.id?.toString() || '0';
    const { discussions } = useDiscussions({
        contributionId: idea?.id || 0,
        perPage: 10,
        page: 1,
    });
    ...
    return (
        <>
            {idea && (
                <SEO
                    title={idea.title}
                    description={(idea.content?.thought || idea.content?.problem || '').substring(0, 200)}
            ...
            >
                ...
                <Box sx={{ p: 2, pt: 0 }}>
                    ...
                    {(() => {
                        ...
                        if (idea?.content?.references) {
                            try {
                                references =
                                    typeof idea.content.references === 'string' ? JSON.parse(idea.content.references) : idea.content.references;
                ...
                <DiscussionSection contributionId={idea?.id || 0} />
                ...
                <ShareBottomSheet
                    ...
                    title="Share Idea"
                    shareUrl={`${window.location.origin}/share/idea/${slug}`}
                    shareText={idea?.title || 'Check out this idea!'}
                />
