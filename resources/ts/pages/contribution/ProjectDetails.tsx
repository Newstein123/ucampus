...
import {
    Box, Typography, Avatar, Chip, Paper, CardMedia, Divider, Button, IconButton, List, ListItem, ListItemAvatar, ListItemText, TextField
} from '@mui/material';
...
const mockProject = {
    ...
    description: 'StudySync is a web-based platform where university students can create and join study groups based on their subjects. It encourages peer-to-peer learning through features like notes, group chats, and scheduled virtual study sessions.',
    problem: 'Many students struggle with understanding complex topics, especially during exam time. However, asking for help can be intimidating, and not everyone can afford private tutors.',
    solution: 'StudySync allows students to help each other by forming interest-based groups. They can share learning materials, clarify doubts, and stay motivated together.',
    whoBenefits: 'University students in any major, especially those in large classrooms or online courses, where personal attention is limited, as it helps introverted students build study relationships at their own pace.',
    resources: [
        'Frontend: React.js',
        'Backend: Firebase / Supabase',
        'UI Design: Figma',
        'Need 1 frontend dev and 1 UI/UX designer to collaborate',
    ],
    attachments: [
    ...
    nextSteps: [
        ...
        'Launch MVP for 1 university by October',
    ...
    return (
        <Layout>
            <Box sx={{ maxWidth: 600, mx: 'auto', bgcolor: '#f7fafd', minHeight: '100vh', p: 2 }}>
                ...
                {/* Title & Author */}
                <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.5, lineHeight: 1.3 }}>{mockProject.title}</Typography>
                ...
                <Box sx={{ border: '1px dashed #b2dfdb', borderRadius: 2, p: 2, mb: 2 }}>
                    ...
                    <ul style={{ margin: 0, paddingLeft: 18 }}>
                        {mockProject.resources.map((r, i) => (
                            <li key={i} style={{ color: '#444', fontSize: 14, marginBottom: 2 }}>{r}</li>
                        ))}
                    </ul>
                </Box>
                {/* Attachments */}
                ...
                <Box sx={{ border: '1px dashed #b2dfdb', borderRadius: 2, p: 2, mb: 2 }}>
                    ...
                    <ul style={{ margin: 0, paddingLeft: 18 }}>
                        {mockProject.nextSteps.map((step, i) => (
                            <li key={i} style={{ color: '#444', fontSize: 14, marginBottom: 2 }}>{step}</li>
                        ))}
                    </ul>
                </Box>
                {/* Stats */}
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 1, py: 1, borderRadius: 2, border: '1px solid #e0e0e0', mb: 2 }}>
                ...
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2, bgcolor: '#fff', mb: 2 }}>
                    ...
                    <List>
                        {mockDiscussion.map((d) => (
                            <ListItem key={d.id} alignItems="flex-start">
                                ...
                                <ListItemText
                                    primary={<Typography sx={{ fontWeight: 600, fontSize: 14 }}>{d.user.name} <span style={{ color: '#888', fontWeight: 400, fontSize: 12 }}>· {d.postedAgo}</span></Typography>}
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <TextField
                            ...
                            disabled
                        />
                        <Button variant="contained" sx={{ bgcolor: '#1F8505', color: '#fff', borderRadius: 2, fontWeight: 600 }} disabled>Post</Button>
                    </Box>
                </Box>
            </Box>
        </Layout>
