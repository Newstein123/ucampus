...
const ProjectDetails: React.FC = () => {
    ...
    const { data: projectResponse, isLoading, refetch } = useContributionDetailQuery(slug || '');
    const project = projectResponse?.data;
    // Derive id from project for backwards compatibility with existing code
    const id = project?.id?.toString() || '0';
        ...
        <>
                ...
                <SEO
                    ...
                    description={
                        typeof project.content === 'string'
                            ? (JSON.parse(project.content)?.description || JSON.parse(project.content)?.problem || '').substring(0, 200)
                            : (project.content?.description || project.content?.problem || '').substring(0, 200)
                    }
            ...
            >
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1, p: 2 }}>
                    {project?.tags?.map((tag) => (
                        <Chip
                            ...
                            sx={{ bgcolor: '#e8f5e9', color: '#1F8505', fontWeight: 600, fontSize: 13 }}
                        />
                    ))}
                </Box>
                ...
                <Box sx={{ p: 2, pt: 0 }}>
                    ...
                    {(() => {
                        ...
                        if (project?.content?.references) {
                            try {
                                references =
                                    typeof project.content.references === 'string'
                                        ? JSON.parse(project.content.references)
                ...
                <ShareBottomSheet
                    ...
                    title="Share Project"
                    shareUrl={`${window.location.origin}/share/project/${slug}`}
                    shareText={project?.title || 'Check out this project!'}
                />
            </SinglePageLayout>
        </>
    );
